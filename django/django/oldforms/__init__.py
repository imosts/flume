...
class BigIntegerField(IntegerField):
    def __init__(self, field_name, length=5, maxlength=5, is_required=False, validator_list=None):
        ...
        oldforms.IntegerField.__init__(self, field_name, length, maxlength, is_required, validator_list)

    def isBigInteger(self, field_data, all_data):
        minval = -9223372036854775808
        maxval = 9223372036854775807
        if not minval <= int(field_data) <= maxval:
            raise validators.CriticalValidationError, gettext("Enter a whole number between %d and %d." % (minval, maxval))
