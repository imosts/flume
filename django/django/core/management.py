...
def labeled_execute (model, connection, sql):
    # Figure out our labelling, setup DB connection and send query.
    epls, desls = model.get_create_labeling ()
        ...
        if col_type is not None:
            ...
            if f.rel:
                if f.rel.to in known_models:
                    pass
                    # flumedb does not support foreign keys
...
def _emit_post_sync_signal(created_models, verbosity, interactive):
    ...
    for app in models.get_apps():
        ...
        if verbosity >= 2:
            print >> outfile2, "Running post-sync handlers for application", app_name
...
def syncdb(verbosity=1, interactive=True):
    ...
    for app in models.get_apps():
        ...
        for model in model_list:
            ...
            if verbosity >= 2:
                print >> outfile2, "Processing %s.%s model" % (app_name, model._meta.object_name)
            ...
            if verbosity >= 1:
                print >> outfile2, "Creating table %s" % model._meta.db_table
            for statement in sql:
                labeled_execute (model, connection, statement)
    ...
    for app in models.get_apps():
        ...
        for model in model_list:
            if model in created_models:
                ...
                if sql:
                    if verbosity >= 2:
                        print >> outfile2, "Creating many-to-many tables for %s.%s model" % (app_name, model._meta.object_name)
                    for statement in sql:
                        labeled_execute (model, connection, statement)
    ...
    for app in models.get_apps():
        for model in models.get_models(app):
            if model in created_models:
                ...
                if custom_sql:
                    if verbosity >= 1:
                        print >> outfile2, "Installing custom SQL for %s.%s model" % (app_name, model._meta.object_name)
                    try:
                        for sql in custom_sql:
                            labeled_execute (model, connection, sql)
                    except Exception, e:
    ...
    for app in models.get_apps():
        ...
        for model in models.get_models(app):
            if model in created_models:
                ...
                if index_sql:
                    if verbosity >= 1:
                        print >> outfile2, "Installing index for %s.%s model" % (app_name, model._meta.object_name)
                    try:
...
The full error: """ % (app_name, app_name)) + style.ERROR_OUTPUT(str(e)) + '\n')
        ...
        print >> outfile2, "Reset cancelled."
...
The full error: """ % settings.DATABASE_NAME + style.ERROR_OUTPUT(str(e)) + '\n'))
        ...
        print >> outfile2, "Flush cancelled."
...
def runserver(addr, port, use_reloader=True, admin_media_dir=''):
    ...
    def inner_run():
        from django.conf import settings
        print >> outfile2, "Validating models..."
        validate()
        print >> outfile2, "\nDjango version %s, using settings %r" % (get_version(), settings.SETTINGS_MODULE)
        print >> outfile2, "Development server is running at http://%s:%s/" % (addr, port)
        print >> outfile2, "Quit the server with %s." % quit_command
        try:
            import django
...
def load_data(fixture_labels, verbosity=1):
    ...
    for fixture_label in fixture_labels:
        if verbosity > 0:
            print >> outfile2, "Loading '%s' fixtures..." % fixture_label
        for fixture_dir in app_fixtures + list(settings.FIXTURE_DIRS) + ['']:
            if verbosity > 1:
                print >> outfile2, "Checking %s for fixtures..." % humanize(fixture_dir)
            ...
            for format in formats:
                ...
                if verbosity > 1:
                    print >> outfile2, "Trying %s for %s fixture '%s'..." % \
                ...
                try:
                    ...
                    if label_found:
                        fixture.close()
                        print >> outfile2, style.ERROR("Multiple fixtures named '%s' in %s. Aborting." % 
                        ...
                        count[1] += 1
                        if verbosity > 0:
                            print >> outfile2, "Installing %s fixture '%s' from %s." % \
                    ...
                    if verbosity > 1:
                        print >> outfile2, "No %s fixture '%s' in %s." % \
                            (format, fixture_name, humanize(fixture_dir))
    if count[0] == 0:
        if verbosity > 0:
            print >> outfile2, "No fixtures found."
    else:
        if verbosity > 0:
            print >> outfile2, "Installed %d object(s) from %d fixture(s)" % tuple(count)
    transaction.commit()
    transaction.leave_transaction_management()
...
def execute_from_command_line(action_mapping=DEFAULT_ACTION_MAPPING, argv=None):
    ...
    if action == 'shell':
        ...
        try:
            print >> outfile2, action_mapping[action](args[1:], options.format, options.indent)
        except IndexError:
            parser.print_usage_and_exit()
        ...
        print >> outfile2, action_mapping[action](args[1:])
        ...
        if action not in NO_SQL_TRANSACTION:
            print >> outfile2, style.SQL_KEYWORD("BEGIN;")
        for mod in mod_list:
            if action == 'reset':
            ...
            if output:
                print >> outfile2, '\n'.join(output)
        if action not in NO_SQL_TRANSACTION:
            print >> outfile2, style.SQL_KEYWORD("COMMIT;")

def setup_environ(settings_mod, project_directory=None):
    """
    ...
    # sys.frozen doesn't seem to work, so try this:
    if __file__ == '<frozen>':
...
def set_output (outfile):
    import __builtin__
    __builtin__.globals ()['outfile2'] = outfile
