...
from django.db.models.options import Options, AdminOptions, FlumeOptions
...
class Model(object):
    ...
    def add_to_class(cls, name, value):
        if name == 'Admin':
            ...
            value = FlumeOptions(**dict([(k, v) for k, v in value.__dict__.items() if not k.startswith('_')]))
    ...
    def save(self, epls=None, desls=None):
        ...
        # Setup connection according to table labels
        epls, desls = self.get_write_labeling (epls, desls)
        ...
        if not pk_set or not record_exists:
            tries = 0
            saved = False
            while tries < 10 and not saved:
                tries += 1
                saved = True
                try:
                    # If primary key is not set, set it randomly.  It
                    # appears that this code assumes there is only one
                    # AutoField, and it is the primary key.
                    if not pk_set:
                        pk_random = True
                        autofields = [f for f in self._meta.fields if isinstance(f, AutoField)]
                        if len (autofields) != 1:
                            raise AssertionError, "I thought there was only one AutoField, not %d!" % len (autofields)
                        import random
                        setattr (self, autofields[0].attname, random.randint(1, 0x7ffffff))
                    ...
                    # We already set the field to a random number, don't get the last insert_id from the DB
            ...
            if not saved:
                raise AssertionError, "Could not save object!"
    ...
    def _flumemeta_getattr (cls, attrname):
        if hasattr (cls, '_flumemeta') and hasattr (cls._flumemeta, attrname):
            return getattr (cls._flumemeta, attrname)
        return None
    _flumemeta_getattr = classmethod(_flumemeta_getattr)

    def get_write_labeling (cls, _epls=None, _desls=None):
        """
        Returns (epls, desls) for saving objects
        according to this model.

        The S and I labels in epls should be assigned to our EP to
        the DB.  The O label in epls should be sent to the DB
        through the MYOLABEL command.
        """
        import flume
        import flume.flmos as flmo

        tablelsd = cls._flumemeta_getattr ('tablelsd')
        if tablelsd:
            epls, desls, sqlprefix = tablelsd.get_write_labeling
        else:
            epls = flmo.get_labelset ()

            import wikicode, wikicode.util
            if wikicode.IMODE & wikicode.IMODE_USER:
                # Always set I label to contain user and developer's Itags
                import wikicode.db.user as w5user
                w5u = w5user.client_from_env ()
                if w5u:
                    ui = w5u.script_tag ()
                    epls.set_I (flmo.Label ([ui]))

            desls = epls.clone ()

            # Only send the capabilities we are using
            typs = "SO"
            if wikicode.IMODE & wikicode.IMODE_USER:
                typs += "I"
            add_o = wikicode.util.get_additional_caps (epls, desls, typs)
            epls.set_O (epls.get_O () + add_o)
        ...
        if False:
            import sys
            sys.stderr.write ('EPLS  %s\n' % epls)
            sys.stderr.write ('_EPLS  %s\n' % _epls)
            sys.stderr.write ('DESLS %s\n' % desls)

        return epls, desls
    get_write_labeling = classmethod(get_write_labeling)

    def get_create_labeling (cls):
        """ Returns (epls, desls) for creating this
        model's table"""
        import flume
        import flume.flmos as flmo
        ...
        if tablelsd:
            ...
            # We could clear verify O label, but verifying capabilities
            # never hurts
            #epls.set_O ()

        return epls, desls
    get_create_labeling = classmethod(get_create_labeling)
        
    def get_read_labeling (cls, itag_requirements=[]):
        """ Returns (epls, desls) for reading objects according to
        this model."""
        import flume
        import flume.flmos as flmo
        ...
        tablelsd = cls._flumemeta_getattr ('tablelsd')
        if tablelsd:
            epls, desls, sqlprefix = tablelsd.get_read_labeling ()
        else:
            epls = flmo.get_labelset ()
            epls.set_O ()
            desls = epls.clone ()

        desls.set_I (desls.get_I () + itag_requirements)

        return epls, desls
    get_read_labeling = classmethod(get_read_labeling)
