...
class QuerySet(object):
    ...
    def __init__(self, model=None):
        ...
        self._itag_has = []          # Any data this queryset returns will have all the integrity tags in _itags
        self._itag_owner_col = None
    ...
    def itag_has (self, itag_has):
        return self._clone (_itag_has=itag_has)

    def itag_owner_in_col (self, columnname):
        return self._clone (_itag_owner_col=columnname)
    ...
    def _get_sql_clause(self):
        ...
        # Add itag_column restriction
        if self._itag_owner_col:
            new_tab = quote_only_if_word ('w5_tagvalue')
            if new_tab not in tables:
                tables.append (new_tab)

            tab = backend.quote_name (opts.db_table)
            col = backend.quote_name (self._itag_owner_col)
            where.append ("(%s ITAGEQ w5_tagvalue.tagvalue AND w5_tagvalue.user_id=%s.%s)"
                          % (tab, tab, col))
...
def delete_objects(seen_objs):
    ...
    epls, desls = self.model.get_write_labeling ()
    cursor = connection.cursor (epls, desls)
