...
def flume_cookie(request, **kw):
    """
    If we got a FLUME_UID, user's already logged in.  else, we got a
    un/pw, to a flume_login.  If the user tries to give us someone
    else's uid, we won't be able to read the other user's user profile
    and will not be able to login.
    """
    user_obj = kw.get('user_obj')

    import os
    uid = os.environ.get ("FLUME_UID")
    if uid:
        u = user.User(request, id=uid,
                      auth_method='flume_cookie', auth_attribs=())

        if u.valid:
            #set_flume_cookie(request, u) # refreshes cookie lifetime
            return u, True # use True to get other methods called, too
    return user_obj, True
