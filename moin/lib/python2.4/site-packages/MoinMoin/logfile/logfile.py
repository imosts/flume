...
class LogFile:
    ...
    def __init__(self, filename, buffer_size=65536, labelset=None):
        """
    ...
    def __getattr__(self, name):
        ...
        if name=="_LogFile__rel_index":
            ...
            if os.path.exists (self.__filename):
                f = flmo.open (self.__filename, 'wa', 0755)
            else:
                from MoinMoin.util import filesys
                d, f = os.path.split (self.__filename)
                filesys.makeDirs (d, labelset=self._ls)
                f = flmo.open (self.__filename, 'cw', 0755, labelset=self._ls)
            self._output = codecs.EncodedFile(f, config.charset)
            return self._output
        else:
