...
def do_show(pagename, request):
    ...
    if request.form.has_key('rev'):
        try:
            ...
            rev = 0
        Page(request, pagename, rev=rev).send_page(request, count_hit=request.cfg.count_hits)
    else:
        request.cacheable = 1
        Page(request, pagename).send_page(request, count_hit=request.cfg.count_hits)
...
def do_edit(pagename, request):
    _ = request.getText
    form = request.form
    if form.has_key('button_preview') and form.has_key('button_spellcheck'):
    ...
    # Save new text
    elif request.form.has_key('button_save'):
        import flume, flume.flmos as flmo
        try:
            savemsg = pg.saveText(savetext, rev, trivial=trivial, comment=comment)
            flmo.set_label(flume.LABEL_O) # Clear O label right afterwards to minimize privileged code
...
Have a look at the diff of %(difflink)s to see what has been changed.""") % {
        ...
        import error
        raise error.UnsupportedError, "You pressed an unknown button on the editor page"
