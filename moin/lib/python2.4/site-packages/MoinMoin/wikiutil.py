...
import os, re, urllib, cgi, time, imp
import codecs
...
def importTheme(theme_name, cfg):
    Theme = None;
    try:
        imp.acquire_lock ()
        theme_integrity = cfg.themes[theme_name].freeze().armor32()
        theme_dir = os.path.join (cfg.theme_dir, theme_integrity)
        fp, path, info = imp.find_module (theme_name, [os.path.abspath(theme_dir)])
        try:
            Theme = getattr(imp.load_module (theme_name, fp, path, info), "Theme")
        finally:
            if fp:
                fp.close()
    finally:
