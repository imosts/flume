...
class Page:
    ...
    def __init__(self, request, page_name, **keywords):
        ...
        self._readergroups = None
        self.labelset, junk = self.split_name (page_name)
        if len (self.labelset.get_I ().toList()) == 0:
            self.labelset.set_I (self.cfg.moin_i_lab) # default to moin
    ...
    def split_name (self, name):
        ...
        if s:
            try:
                ls.set_S (flumeclnt.uname2label (s, self.request.cfg.usermgruid))
                self._readergroups = [s]
            except flume.DeclassificationError, err:
                pass
            except OSError, err:
                pass
        if i:
            if self.request.cfg.themes.has_key(i):
                ls.set_I (self.request.cfg.themes[i])
            else:
                self.request.log ("got unexpected integrity label %s, ignoring" % i)
    ...
    def reset(self):
        ...
        if self.is_rootpage: # we have no request.rootpage yet!
            if not page_name:
                normalpath = os.path.join (self.cfg.page_dir, self.labelset.to_filename ())
            else:
                raise NotImplementedError(
                    "TODO: handle other values of rootpage (not used yet)")
        else:
            normalpath = self.request.rootpage.getPagePath(self.labelset.to_filename (),
                                                           qpagename,
        ...
        # self is rootpage
            ...
            # our current rootpage is THE virtual rootpage, really at top of all
                ...
                # no need to check 'standard' case, we just use path in that case!
                    ...
                    underlay, path = 0, self.cfg.page_dir
    ...
    def lastEditInfo(self, request=None):
        ...
        if self.request.cfg.show_last_editor:
            ...
            return {'editor': "Undefined", 'time': "Undefined" }
    ...
    def _listPages(self):
        ...
        if self.cfg.data_underlay_dir is not None:
            ...
            path = self.getPagePath(use_underlay=1)
    ...
    def send_page_content(self, request, Parser, body, format_args='',
        ...
        if not (do_cache and self.canUseCache(Parser)):
            request.log ("cache disabled! generating on the fly")
            self.format(parser)
        else:
            try:
                ...
                if True:
                    try:
                        request.log ("cache miss! generating on the fly and saving")
                    ...
                    request.log ("cache miss! generating on the fly")
                    self.format(parser)
    ...
    def _emptyPageText(self, request):
        ...
        output = """
        <p><strong>This page does not exist yet. You can create a new empty
        page.</strong></p>
        """

        if self._readergroups:
            readg = ", ".join(self._readergroups)
        else:
            readg = ""

        output += "<p>Reader must have these privileges: {%s}</p>\n" % readg

        output += "<form method='get' action=%s>" % request.request_uri
        output += "<p>Allow these principals to modify the page:<br>"
        output += "<input type='text' name='allowwrite' value='%s'><BR>" % readg
        output += "</p>\n\n"

        output += "<p><input type='submit' name='create' value='Create'></p>\n"
