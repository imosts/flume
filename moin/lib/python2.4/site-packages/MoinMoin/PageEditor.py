...
class PageEditor(Page):
    ...
    def __init__(self, request, page_name, **keywords):
        ...
        self.write_ls = flmo.LabelSet (self.labelset.toDict ())
        if self.request.args.has_key ("allowwrite"):
            writers = self.request.args["allowwrite"][0]
            writers = writers.split(",")
            olabel = flmo.Label ()
            for w in writers:
                if (len (w) > 0):
                    self.allowed_writers.append (w)
                    olabel += flumeclnt.uname2wtag (w, self.request.cfg.usermgruid)
            self.write_ls.set_O (olabel)

            pagedir = self.getPagePath(use_underlay=0, check_create=0)
            if os.path.exists(pagedir):
                ls = flmo.stat_file (pagedir)
                if not (self.write_ls.get_O () == ls.get_O ()):
                    import error
                    raise error.UnsupportedError, \
                          "Page exists, you can't change the O label from %s to %s.  \
                          Edit the exising page." % (ls.get_O (), self.write_ls.get_O ())
...
''' % _("Your changes are not saved!"))
    ...
    def sendEditor(self, **kw):
        ...
        #    msg = _('Page is immutable!')
            ...
            try:
                # make sure the ihome directory is made with the right labels
                ihomedir, pagedir = os.path.split (self.getPagePath (use_underlay=0, check_create=0))
                if not os.path.exists (ihomedir):
                    filesys.makeDirs (ihomedir, labelset=self.labelset)
            ...
            except flume.PermissionError, err:
                msg = _("Permission Error, you're probably not allowed to modify this "
                        "page.  %s" % str(err))
...
Have a look at the diff of %(difflink)s to see what has been changed.""") % {
        ...
        if preview and raw_body and self.request.cfg.make_backups:
        ...
        # Figure out which writers are allowed for this page, and send when user hits "Save Changes"
        self.request.write ('<input type="hidden" name="allowwrite" value="%s">' % ','.join(self.allowed_writers))
...
''' % {
    ...
    def sendCancel(self, newtext, rev):
        ...
        """
        _ = self._
        if self.request.cfg.make_backups:
...
Try a different name.""") % (newpagename,)
    ...
    def _write_file(self, text, action='SAVE', comment=u'', extra=u''):
        ...
        cachedir = os.path.join(pagedir, 'cache')
        ...
        llog = editlog.EditLog(self.request, filename=pagelog, labelset=self.write_ls,
                               uid_override=self.uid_override, )
        # Open the global log
        if self.request.cfg.use_global_event_log:
        ...
        if not os.path.exists(pagedir): # new page, create and init pagedir
            filesys.makeDirs(pagedir, labelset=self.write_ls)

        if not os.path.exists(cachedir):
            filesys.makeDirs(cachedir, labelset=self.write_ls)
        if not os.path.exists(revdir):        
            flmo.mkdir(revdir, 0755, labelset=self.write_ls)

            f = flmo.open(cfn, 'wc', 0755, labelset=self.write_ls)
            f.write('%08d\n' % 0)
            f.close()
        ...
        try:
                ...
                if not was_deprecated:
                    if self.do_revision_backup or rev == 0:
                        rev += 1
                revstr = '%08d' % rev
                f = flmo.open(clfn, 'w', 0755, labelset=self.write_ls)
                f.write(revstr+'\n')
                f.close()
                
                # save to page file
                pagefile = os.path.join(revdir, revstr)
                f = flmo.open (pagefile, 'wc', 0755, labelset=self.write_ls)
                f = codecs.EncodedFile(f, config.charset)
...
To leave the editor, press the Cancel button.""") % {
    ...
    def _readLockFile(self):
        ...
        if self.locktype:
            try:
                entry = editlog.EditLog(self.request,
                                        filename=self._filename(),
                                        labelset=self.pageobj.write_ls).next()
    ...
    def _writeLockFile(self):
        ...
        self._deleteLockFile()
        try:
            editlog.EditLog(self.request,
                            filename=self._filename(),
                            labelset=self.pageobj.write_ls).add(
