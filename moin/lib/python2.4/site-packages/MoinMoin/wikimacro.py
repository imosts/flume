    ...
    def __get_Date(self, args, format_date):
        ...
        if not args:
            ...
            try:
                ...
                flm = args[19:] # +HHMM, -HHMM or Z or nothing (then we assume Z)
                flmoffset = 0 # we assume UTC no matter if there is a Z
                if flm:
                    sign = flm[0]
                    if sign in '+-':
                        flmh, flmm = int(flm[1:3]), int(flm[3:])
                        flmoffset = (flmh*60+flmm)*60
                        if sign == '-':
                            flmoffset = -flmoffset
            ...
            try:
                tm = time.mktime(tm) - time.timezone - flmoffset
