# -*- coding: utf-8 -*-
# Text translations for Português do Brasil (pt_br).
# Automatically generated - DO NOT EDIT, edit pt_br.po instead!
meta = {
  'language': """Português do Brasil""",
  'elanguage': """Brazillian Portuguese""",
  'maintainer': """Leonardo Gregianin <leonardo@riseup.net>""",
  'encoding': 'utf-8',
  'direction': 'ltr',
  'wikimarkup': True,
}
text = {
'''The backed up content of this page is deprecated and will not be included in search results!''':
'''O conteúdo de backup desta página é velho e não será incluso nos resultados das buscas!''',
'''Revision %(rev)d as of %(date)s''':
'''Revisão %(rev) de %(date)s''',
'''Redirected from page "%(page)s"''':
'''Redirecionado da página "%(page)s"''',
'''This page redirects to page "%(page)s"''':
'''Esta página redireciona para a página "%(page)s"''',
'''~-If you submit this form, the submitted values will be displayed.
To use this form on other pages, insert a
[[BR]][[BR]]\'\'\'{{{    [[Form("%(pagename)s")]]}}}\'\'\'[[BR]][[BR]]
macro call.-~
''':
'''~-Se você enviar este formulário, os valores enviados serão mostrados.
Para usar este formulário em outras páginas, insira uma macro do tipo
[[BR]][[BR]]\'\'\'{{{    [[Form("%(pagename)s")]]}}}\'\'\'[[BR]][[BR]]-~
''',
'''Create New Page''':
'''Criar Nova Página''',
'''You are not allowed to view this page.''':
'''Você não tem permissão para ver esta página!''',
'''You are not allowed to edit this page.''':
'''Você não tem permissão para editar esta página!''',
'''Page is immutable!''':
'''A página não é editável!''',
'''Cannot edit old revisions!''':
'''Não é possível editar revisões antigas!''',
'''The lock you held timed out. Be prepared for editing conflicts!''':
'''Seu tempo de edição se esgotou. Esteja preparado para editar conflitos!''',
'''Edit "%(pagename)s"''':
'''Editar "%(pagename)s"''',
'''Preview of "%(pagename)s"''':
'''Previsão de "%(pagename)s"''',
'''Your edit lock on %(lock_page)s has expired!''':
'''Seu tempo de edição para %(lock_page)s está esgotado!''',
'''Your edit lock on %(lock_page)s will expire in # minutes.''':
'''Seu tempo de edição para %(lock_page)s irá se esgotar em # minutos!''',
'''Your edit lock on %(lock_page)s will expire in # seconds.''':
'''Seu tempo de edição para %(lock_page)s irá se esgotar em # segundos''',
'''Someone else deleted this page while you were editing!''':
'''Alguém apagou esta página enquanto você a editava!''',
'''Someone else changed this page while you were editing!''':
'''Alguém alterou esta página enquanto você a editava!''',
'''Someone else saved this page while you were editing!
Please review the page and save then. Do not save this page as it is!
Have a look at the diff of %(difflink)s to see what has been changed.''':
'''Alguém alterou esta página enquanto você a editava!!
Por favor, revise a página antes de salvá-la. Não a salve assim como está!
Confira as alterações de %(difflink)s para verificar o que foi alterado''',
'''[Content of new page loaded from %s]''':
'''[Conteúdo da nova página carregado de %s]''',
'''[Template %s not found]''':
'''[Template %s não encontrado]''',
'''[You may not read %s]''':
'''[Você não tem permissão para ler %s]''',
'''Describe %s here.''':
'''Descreva %s aqui.''',
'''Check Spelling''':
'''Verificar ortografia''',
'''Save Changes''':
'''Salvar Mudanças''',
'''Cancel''':
'''Cancelar''',
'''By hitting \'\'\'%(save_button_text)s\'\'\' you put your changes under the %(license_link)s.
If you don\'t want that, hit \'\'\'%(cancel_button_text)s\'\'\' to cancel your changes.''':
'''Ao apertar o botão \'\'\'%(save_button_text)s\'\'\' você põe suas alterações sob a %(license_link)s.
Se você discorda, aperte o botão \'\'\'%(cancel_button_text)s\'\'\' para cancelar suas alterações.''',
'''Preview''':
'''Previsão''',
'''Text mode''':
'''Modo texto''',
'''Comment:''':
'''Comentário:''',
'''<No addition>''':
'''<Sem adição>''',
'''Add to: %(category)s''':
'''Adição de: %(category)s''',
'''Trivial change''':
'''Alterações superficiais''',
'''Remove trailing whitespace from each line''':
'''Remover espaço em branco no final de cada linha''',
'''Invalid user name {{{\'%s\'}}}.
Name may contain any Unicode alpha numeric character, with optional one
space between words. Group page name is not allowed.''':
'''Nome de usuário inválido {{{\'%s\'}}}.
O nome pode conter qualquer caractere alfanumérico Unicode, com um espaço opcional
entre as palavras. Nome de grupo de página não é permitido.''',
'''You are not allowed to do %s on this page.''':
'''Você não tem permissão para fazer %s nesta página.''',
'''%(hits)d results out of about %(pages)d pages.''':
'''%(hits)d páginas localizadas de %(pages)d páginas.''',
'''%.2f seconds''':
'''%.2f segundos''',
'''match''':
'''combina''',
'''matches''':
'''combinam''',
'''<unknown>''':
'''<desconhecido>''',
'''Login Name: %s

Login Password: %s

Login URL: %s/%s?action=login
''':
'''Nome de Login: %s

Senha do Nome: %s

URL do Login: %s/%s?action=login
''',
'''Somebody has requested to submit your account data to this email address.

If you lost your password, please use the data below and just enter the
password AS SHOWN into the wiki\'s password form field (use copy and paste
for that).

After successfully logging in, it is of course a good idea to set a new and known password.
''':
'''Você solicitou submeter seus dados de conta a este endereço de e-mail.

Se você perder sua senha, por favor use os dados acima e somente entre com
a senha COMO MOSTRADO no campo da senha do wiki (utilize copiar e colar
para isto).

Depois de logar com sucesso, seria uma boa idéia solicitar uma nova senha.
''',
'''[%(sitename)s] Your wiki account data''':
'''[%(sitename)s] Seus dados da conta do wiki''',
'''This wiki is not enabled for mail processing.
Contact the owner of the wiki, who can enable email.''':
'''Esta wiki não está habilitada a enviar mensagens de correio eletrônico.
Contate o administrador desta wiki para habilitar esta opção.''',
'''Please provide a valid email address!''':
'''Por favor, forneça um endereço de e-mail válido!''',
'''Found no account matching the given email address \'%(email)s\'!''':
'''Não foi encontrada nenhuma conta com o endereço \'%(email)s!''',
'''Use UserPreferences to change your settings or create an account.''':
'''Use as PreferênciasDoUsuário para mudar suas configurações ou criar uma outra conta.''',
'''Empty user name. Please enter a user name.''':
'''Nome do usuário não informado. Por favor, informe o nome do usuário.''',
'''This user name already belongs to somebody else.''':
'''Este nome de usuário já pertence a alguém cadastrado neste site.''',
'''Passwords don\'t match!''':
'''As senhas não conferem!''',
'''Please specify a password!''':
'''Por favor, informe uma senha!''',
'''Please provide your email address. If you lose your login information, you can get it by email.''':
'''Por favor informe um endereço de email. Se você perder sua informação de login, você poderá recuperá-la por email.''',
'''This email already belongs to somebody else.''':
'''Este email já pertence a alguém cadastrado neste site.''',
'''User account created! You can use this account to login now...''':
'''Conta de usuário criada! Você pode usar esta conta para se logar agora...''',
'''Use UserPreferences to change settings of the selected user account''':
'''Use as PreferênciasDoUsuário para mudar suas configurações da conta do usuário.''',
'''The theme \'%(theme_name)s\' could not be loaded!''':
'''Não foi possível carregar o tema \'%(theme_name)s\'!''',
'''User preferences saved!''':
'''Preferências do usuário salvas!''',
'''Default''':
'''Padrão''',
'''<Browser setting>''':
'''<Configuração do navegador>''',
'''the one preferred''':
'''o qual for preferido''',
'''free choice''':
'''escolha livre''',
'''Select User''':
'''Selecionar Usuário''',
'''Save''':
'''Salvar''',
'''Preferred theme''':
'''Tema preferido''',
'''Editor Preference''':
'''Preferências do Editor''',
'''Editor shown on UI''':
'''Mostrar editor gráfico''',
'''Time zone''':
'''Fuso horário''',
'''Your time is''':
'''Sua hora é''',
'''Server time is''':
'''A hora no servidor é''',
'''Date format''':
'''Formato da data''',
'''Preferred language''':
'''Língua preferida''',
'''General options''':
'''Opções gerais''',
'''Quick links''':
'''Links rápidos''',
'''This list does not work, unless you have entered a valid email address!''':
'''Esta lista não funcionará exceto se você inseriu um email válido!''',
'''Subscribed wiki pages (one regex per line)''':
'''Páginas subscritas na wiki (uma regex, expressão regular, por linha)''',
'''Create Profile''':
'''Criar Perfil''',
'''Mail me my account data''':
'''Envie os dados de minha conta por email''',
'''Email''':
'''Email''',
'''To create an account or recover a lost password, see the %(userprefslink)s page.''':
'''Para criar uma conta ou recuperar uma senha perdida, veja a página %(userprefslink)s.''',
'''Name''':
'''Nome''',
'''Password''':
'''Senha''',
'''Login''':
'''Login''',
'''Action''':
'''Ação''',
'''Required attribute "%(attrname)s" missing''':
'''O atributo necessário "%(attrname)s" está faltando.''',
'''Submitted form data:''':
'''Formulário de dados enviado:''',
'''Search Titles''':
'''Buscar Títulos''',
'''Display context of search results''':
'''Mostrar contexto dos resultados da busca''',
'''Case-sensitive searching''':
'''Buscar diferenciando maiúsculas e minúsculas''',
'''Search Text''':
'''Procurar Texto''',
'''Go To Page''':
'''Ir Para Página''',
'''Include system pages''':
'''Incluir páginas de sistema''',
'''Exclude system pages''':
'''Excluir páginas de sistema''',
'''Plain title index''':
'''Índice de texto puro''',
'''XML title index''':
'''Índice de títulos em XML''',
'''Python Version''':
'''Versão Python''',
'''MoinMoin Version''':
'''Versão MoinMoin''',
'''Release %s [Revision %s]''':
'''Liberar %s [Revisão %s]''',
'''4Suite Version''':
'''Versão 4Suite''',
'''Number of pages''':
'''Número de páginas''',
'''Number of system pages''':
'''Número de páginas de sistema''',
'''Accumulated page sizes''':
'''Total acumulado dos tamanhos de páginas''',
'''Disk usage of %(data_dir)s/pages/''':
'''Uso do disco de %(data_dir)s/pages/''',
'''Disk usage of %(data_dir)s/''':
'''Uso do disco de %(data_dir)s/''',
'''Entries in edit log''':
'''Entradas no registro de edição''',
'''NONE''':
'''NENHUM''',
'''Global extension macros''':
'''Macros de extensão global''',
'''Local extension macros''':
'''Macros de extensão local''',
'''Global extension actions''':
'''Ações de extensão global''',
'''Local extension actions''':
'''Ações de extensão local''',
'''Global parsers''':
'''Parsers globais''',
'''Local extension parsers''':
'''Extensão de parsers locais''',
'''Installed processors (DEPRECATED -- use Parsers instead)''':
'''Processadores instalados (OBSOLETO -- utilize Parsers)''',
'''Disabled''':
'''Desabilitado''',
'''Enabled''':
'''Habilitado''',
'''Lupy search''':
'''Busca com Lupy''',
'''Active threads''':
'''Ativar threads''',
'''Please use a more selective search term instead of {{{"%s"}}}''':
'''Por favor, use um termo mais preciso ao invés de {{{"%s"}}}''',
'''ERROR in regex \'%s\'''':
'''ERRO na regex, expressão regular, \'%s\'''',
'''Bad timestamp \'%s\'''':
'''Formato da data está errrado \'%s\'''',
'''Expected "=" to follow "%(token)s"''':
'''Esperado "=" depois de "%(token)s"''',
'''Expected a value for key "%(token)s"''':
'''Esperado um valor para "%(token)s"''',
'''Wiki Markup''':
'''Markup da Wiki''',
'''Print View''':
'''Visualizar Impressão''',
'''Your changes are not saved!''':
'''Suas mudanças não foram gravadas!''',
'''Page name is too long, try shorter name.''':
'''O nome desta página é muito grande, tente um mais curto.''',
'''GUI Mode''':
'''Visualização Gráfica''',
'''Edit was cancelled.''':
'''A edição foi cancelada.''',
'''You can\'t rename to an empty pagename.''':
'''Você não pode renomear páginas vazias.''',
'''\'\'\'A page with the name {{{\'%s\'}}} already exists.\'\'\'

Try a different name.''':
'''\'\'\'Já existe uma página com o nome {{{\'%s\'}}}\'\'\'

Tente um nome diferente.''',
'''Could not rename page because of file system error: %s.''':
'''Não é possível renomear página devido a um erro no sistema de arquivos: %s''',
'''Thank you for your changes. Your attention to detail is appreciated.''':
'''Obrigado por suas mudanças. Sua atenção aos detalhes é apreciada.''',
'''Page "%s" was successfully deleted!''':
'''Página "%s" foi removida com sucesso!''',
'''Dear Wiki user,

You have subscribed to a wiki page or wiki category on "%(sitename)s" for change notification.

The following page has been changed by %(editor)s:
%(pagelink)s

''':
'''Caro usuário Wiki,

Você subscreveu uma página ou categoria em "%(sitename)s" para notificações de alterações.

A página seguinte foi alterada por %(editor)s:
%(pagelink)s

''',
'''The comment on the change is:
%(comment)s

''':
'''O comentário referente à alteração é:
%(comment)s

''',
'''New page:
''':
'''Nova página:
''',
'''No differences found!
''':
'''Nenhuma diferença encontrada!
''',
'''[%(sitename)s] %(trivial)sUpdate of "%(pagename)s" by %(username)s''':
'''[%(sitename)s] %(trivial)sAtualização de "%(pagename)s" por %(username)s''',
'''Trivial ''':
'''Triviais ''',
'''Status of sending notification mails:''':
'''Status de notificação por e-mail:''',
'''[%(lang)s] %(recipients)s: %(status)s''':
'''[%(lang)s] %(recipients)s: %(status)s''',
'''## backup of page "%(pagename)s" submitted %(date)s''':
'''## backup da página "%(pagename)s" submetido em %(date)s''',
'''Page could not get locked. Unexpected error (errno=%d).''':
'''Página não pode ser bloqueada. Erro inesperado (errno=%d).''',
'''Page could not get locked. Missing \'current\' file?''':
'''A página não pode ser bloqueada para edição. Desconsiderar arquivo \'corrente\'?''',
'''You are not allowed to edit this page!''':
'''Você não tem permissão para editar esta página!''',
'''You cannot save empty pages.''':
'''Você não pode salvar páginas vazias.''',
'''You already saved this page!''':
'''Você já salvou esta página!''',
'''Sorry, someone else saved the page while you edited it.

Please do the following: Use the back button of your browser, and cut&paste
your changes from there. Then go forward to here, and click EditText again.
Now re-add your changes to the current page contents.

\'\'Do not just replace
the content editbox with your version of the page, because that would
delete the changes of the other person, which is excessively rude!\'\'
''':
'''Desculpe, mas alguém mais salvou esta página enquanto você a editava

Por favor, faça o seguinte: Use o botão de voltar do seu navegador, e recorte e faça um recortar&colar
de suas mudanças a partir de lá. Depois, avance novamente para esta página, e clique em Editar Texto novamente
Agora, torne a fazer suas alterações nos conteúdos atuais da página.

"Não tente apenas substituir
o conteúdo da caixa de edição com a sua versão da página, pois isto iria
apagar as alterações da outra pessoa, o que é demasiadamente rude!"
''',
'''A backup of your changes is [%(backup_url)s here].''':
'''Uma cópia de segurança das suas alterações está [%(backup_url)s aqui].''',
'''You did not change the page content, not saved!''':
'''Você não pode alterar o conteúdo da página, alterações não foram salvas!''',
'''You can\'t change ACLs on this page since you have no admin rights on it!''':
'''Você não podem mudar ACLs nesta página, pois você não tem direitos de administração sobre ela!''',
'''The lock of %(owner)s timed out %(mins_ago)d minute(s) ago, and you were granted the lock for this page.''':
'''O bloqueio de %(owner)s expirou %(mins_ago)d minuto(s) atrás, e você foi autorizado a bloquear esta página para edição.''',
'''Other users will be \'\'blocked\'\' from editing this page until %(bumptime)s.''':
'''Outros usuários serão impedidos de editar esta página até %(bumptime)s.''',
'''Other users will be \'\'warned\'\' until %(bumptime)s that you are editing this page.''':
'''Outros usuários serão \'\'alertados\'\' até %(bumptime)s que você está editando esta página.''',
'''Use the Preview button to extend the locking period.''':
'''Use o botão Previsão para prorrogar o período de bloqueio para edição.''',
'''This page is currently \'\'locked\'\' for editing by %(owner)s until %(timestamp)s, i.e. for %(mins_valid)d minute(s).''':
'''Está página está atualmente \'\'bloqueada\'\' para edição por %(owner)s até %(timestamp)s, isto é por %(mins_valid)d minuto(s).''',
'''This page was opened for editing or last previewed at %(timestamp)s by %(owner)s.[[BR]]
\'\'\'You should \'\'refrain from editing\'\' this page for at least another %(mins_valid)d minute(s),
to avoid editing conflicts.\'\'\'[[BR]]
To leave the editor, press the Cancel button.''':
'''Esta página foi aberta para edição em %(timestamp)s por %(owner)s.[[BR]]
\'\'\'Você deve \'\'resistir à tentação de editar\'\' esta página ao menos durante %(mins_valid)d minuto(s),
para evitar conflitos de edição.\'\'\'[[BR]]
Para deixar o editor, selecione o botão Cancelar.''',
'''The package needs a newer version of MoinMoin (at least %s).''':
'''O pacote precisa de uma versão mais nova do MoinMoin (mais nova que %s).''',
'''The theme name is not set.''':
'''O nome do tema não está ajustado.''',
'''Installing theme files is only supported for standalone type servers.''':
'''Instalar arquivos de temas só é suportado por servidores.''',
'''Installation of \'%(filename)s\' failed.''':
'''Instalação de \'%(filename)s\' falhou.''',
'''The file %s is not a MoinMoin package file.''':
'''O arquivo %s não é um arquivo de pacote do MoinMoin.''',
'''The page %s does not exist.''':
'''A página %s não existe.''',
'''Invalid package file header.''':
'''O cabeçalho do arquivo é inválido.''',
'''Package file format unsupported.''':
'''O formato do arquivo não é suportado.''',
'''Unknown function %(func)s in line %(lineno)i.''':
'''Função %(func)s não reconhecida na linha %(lineno)i.''',
'''The file %s was not found in the package.''':
'''O arquivo %s não foi encontrado no pacote do MoinMoin.''',
''' Emphasis:: [[Verbatim(\'\')]]\'\'italics\'\'[[Verbatim(\'\')]]; [[Verbatim(\'\'\')]]\'\'\'bold\'\'\'[[Verbatim(\'\'\')]]; [[Verbatim(\'\'\'\'\')]]\'\'\'\'\'bold italics\'\'\'\'\'[[Verbatim(\'\'\'\'\')]]; [[Verbatim(\'\')]]\'\'mixed \'\'[[Verbatim(\'\'\')]]\'\'\'\'\'bold\'\'\'[[Verbatim(\'\'\')]] and italics\'\'[[Verbatim(\'\')]]; [[Verbatim(----)]] horizontal rule.
 Headings:: [[Verbatim(=)]] Title 1 [[Verbatim(=)]]; [[Verbatim(==)]] Title 2 [[Verbatim(==)]]; [[Verbatim(===)]] Title 3 [[Verbatim(===)]];   [[Verbatim(====)]] Title 4 [[Verbatim(====)]]; [[Verbatim(=====)]] Title 5 [[Verbatim(=====)]].
 Lists:: space and one of: * bullets; 1., a., A., i., I. numbered items; 1.#n start numbering at n; space alone indents.
 Links:: [[Verbatim(JoinCapitalizedWords)]]; [[Verbatim(["brackets and double quotes"])]]; url; [url]; [url label].
 Tables:: || cell text |||| cell text spanning 2 columns ||;    no trailing white space allowed after tables or titles.

(!) For more help, see HelpOnEditing or SyntaxReference.
''':
''' Ênfase:: [[Verbatim(\'\')]]\'\'itálico\'\'[[Verbatim(\'\')]]; [[Verbatim(\'\'\')]]\'\'\'negrito\'\'\'[[Verbatim(\'\'\')]]; [[Verbatim(\'\'\'\'\')]]\'\'\'\'\'negrito itálico\'\'\'\'\'[[Verbatim(\'\'\'\'\')]]; [[Verbatim(\'\')]]\'\'misto \'\'[[Verbatim(\'\'\')]]\'\'\'\'\'negrito\'\'\'[[Verbatim(\'\'\')]] e itálico\'\'[[Verbatim(\'\')]]; [[Verbatim(----)]] linha horizontal.
 Títulos:: [[Verbatim(=)]] Título 1 [[Verbatim(=)]]; [[Verbatim(==)]] Título 2 [[Verbatim(==)]]; [[Verbatim(===)]] Título 3 [[Verbatim(===)]];   [[Verbatim(====)]] Título 4 [[Verbatim(====)]]; [[Verbatim(=====)]] Título 5 [[Verbatim(=====)]].
 Listas:: Espaço com um dos seguintes: * marcadores; 1., a., A., i., I. listas numeradas; 1.#n começar numeração em n; espaço sozinho identa a linha.
 Links:: [[Verbatim(PalavrasComIniciaisMaiúsculas)]]; [[Verbatim(["Aspas duplas"])]]; url; [url]; [Título da URL].
 Tabelas:: || texto da célula |||| texto da célula mesclando duas colunas ||;    espaços em branco não são permitidos após tabelas ou títulos.

(!) Para mais ajuda, veja AjudaNaEdição ou ReferênciaDeSintaxe.
''',
'''Diffs''':
'''Diffs''',
'''Info''':
'''Informações''',
'''Edit''':
'''Editar''',
'''UnSubscribe''':
'''RemoverInscrição''',
'''Subscribe''':
'''Subscrever''',
'''Raw''':
'''TextoBruto''',
'''XML''':
'''XML''',
'''Print''':
'''Imprimir''',
'''View''':
'''Visualizar''',
'''Up''':
'''Acima''',
'''Publish my email (not my wiki homepage) in author info''':
'''Publique meu e-mail (não em minha homepage do wiki) nas informações do autor''',
'''Open editor on double click''':
'''Abra o editor com um duplo clique''',
'''Jump to last visited page instead of frontpage''':
'''Ir à última página visitada em vez da página inicial''',
'''Show question mark for non-existing pagelinks''':
'''Mostrar sinal de interrogação nos links cujas páginas não existem''',
'''Show page trail''':
'''Mostrar histórico de páginas visitadas''',
'''Show icon toolbar''':
'''Mostrar caixa de ferramentas com ícones''',
'''Show top/bottom links in headings''':
'''Mostrar links para topo/final nos cabeçalhos''',
'''Show fancy diffs''':
'''Útilizar visualização agradável para diffs''',
'''Add spaces to displayed wiki names''':
'''Adicionar espaços aos WikiNome''',
'''Remember login information''':
'''Lembrar das informações de login''',
'''Subscribe to trivial changes''':
'''Subscrever alterações superficiais''',
'''Disable this account forever''':
'''Desabilitar definitivamente esta conta''',
'''(Use Firstname\'\'\'\'\'\'Lastname)''':
'''(Use Nome\'\'\'\'\'\'Sobrenome)''',
'''Alias-Name''':
'''Apelido''',
'''Password repeat''':
'''Repita a senha''',
'''(Only when changing passwords)''':
'''(Apenas para alteração da senha)''',
'''User CSS URL''':
'''URL da folha de estilo (CSS) do usuário''',
'''(Leave it empty for disabling user CSS)''':
'''(Deixe em branco para desabilitar a folha de estil CSS do usuário)''',
'''Editor size''':
'''Tamanho do editor''',
'''No older revisions available!''':
'''Nenhuma revisão antiga disponível!''',
'''Diff for "%s"''':
'''Diferenças (diffs) para "%s"''',
'''Differences between revisions %d and %d''':
'''Diferenças entre as versões de %d e %d''',
'''(spanning %d versions)''':
'''(superpondo %d versões)''',
'''No differences found!''':
'''Nenhuma differença encontrada!''',
'''The page was saved %(count)d times, though!''':
'''A página já foi salva %(count)d vezes!''',
'''(ignoring whitespace)''':
'''(ignorando espaços em branco)''',
'''Ignore changes in the amount of whitespace''':
'''Ingnore diferenças na quantidade de espaços em branco''',
'''General Information''':
'''Informações gerais''',
'''Page size: %d''':
'''Tamanho da página: %d''',
'''SHA digest of this page\'s content is:''':
'''SHA digest para o conteúdo desta página é:''',
'''The following users subscribed to this page:''':
'''Os seguintes usuários se inscreveram nesta página:''',
'''This page links to the following pages:''':
'''Esta página tem links para as seguintes páginas:''',
'''Date''':
'''Data''',
'''Size''':
'''Tamanho''',
'''Diff''':
'''Diff''',
'''Editor''':
'''Editor''',
'''Comment''':
'''Comentário''',
'''view''':
'''visualizar''',
'''raw''':
'''bruto''',
'''print''':
'''imprimir''',
'''revert''':
'''reverter''',
'''Revert to revision %(rev)d.''':
'''Reverter para a revisão %(rev)d.''',
'''edit''':
'''editar''',
'''get''':
'''obter''',
'''del''':
'''del''',
'''N/A''':
'''N/D''',
'''Revision History''':
'''Histórico de Revisões''',
'''No log entries found.''':
'''Nenhum registro de log encontrado.''',
'''Info for "%s"''':
'''Informação para "%s"''',
'''Show "%(title)s"''':
'''Mostrar "%(title)s"''',
'''General Page Infos''':
'''Informações gerais da página''',
'''Show chart "%(title)s"''':
'''Mostrar o gráfico "%(title)s"''',
'''Page hits and edits''':
'''Número de acessos e edições da página''',
'''You are not allowed to revert this page!''':
'''Você não tem permissão para reverter as alterações desta página!''',
'''You must login to add a quicklink.''':
'''Você precisa se logar para adicionar links rápidos.''',
'''Your quicklink to this page has been removed.''':
'''Sua inscrição a esta página foi removida.''',
'''A quicklink to this page has been added for you.''':
'''Uma inscrição a esta página foi adicionada por você.''',
'''You are not allowed to subscribe to a page you can\'t read.''':
'''Você não tem permissão para se inscrever em uma página que você não pode ler.''',
'''This wiki is not enabled for mail processing.''':
'''Este wiki não está habilitado a enviar mensagens de correio eletrônico.''',
'''You must log in to use subscribtions.''':
'''Você precisa fazer o login antes de se inscrever.''',
'''Add your email address in your UserPreferences to use subscriptions.''':
'''Adicione um endereço de e-mail nas PreferênciasDoUsuário para usar as inscrições.''',
'''Your subscribtion to this page has been removed.''':
'''Sua subscrição a esta página foi removida.''',
'''Can\'t remove regular expression subscription!''':
'''Não é possível remover expressão regular correspondendo a subscrição!''',
'''Edit the subscription regular expressions in your UserPreferences.''':
'''Edite suas inscrições de páginas nas PreferênciasDoUsuário.''',
'''You have been subscribed to this page.''':
'''Sua inscrição a esta página foi efetivada.''',
'''Charts are not available!''':
'''Gráficos não estão disponíveis!''',
'''You need to provide a chart type!''':
'''Você precisa informar um tipo de gráfico!''',
'''Bad chart type "%s"!''':
'''Tipo inadequado de gráfico "%s"!''',
'''This page is already deleted or was never created!''':
'''Esta página já está removida ou nunca foi criada!''',
'''No pages like "%s"!''':
'''Nenhuma página como "%s"!''',
'''Invalid filename "%s"!''':
'''Nome do arquivo "%s" inválido!''',
'''Attachment \'%(target)s\' (remote name \'%(filename)s\') already exists.''':
'''Anexo \'%(target)s\' (remote name \'%(filename)s\') já existe.''',
'''Created the package %s containing the pages %s.''':
'''Criado o pacote %s contendo %s páginas.''',
'''Package pages''':
'''Páginas do Pacote''',
'''Package name''':
'''Nome do Pacote''',
'''List of page names - separated by a comma''':
'''Lista de páginas - separadas por vírgula''',
'''Unknown user name: {{{"%s"}}}. Please enter user name and password.''':
'''Nome de usuário desconhecido: {{{"%s"}}}. Por favor, informe o nome do usuário e senha.''',
'''Missing password. Please enter user name and password.''':
'''Senha não foi informada. Por favor, informe o nome do usuário e senha.''',
'''Sorry, wrong password.''':
'''Desculpe. Senha errada.''',
'''Exactly one page like "%s" found, redirecting to page.''':
'''Somente uma página semelhante a "%s" encontrada, redirecionando para a página.''',
'''Pages like "%s"''':
'''Páginas semelhantes "%s"''',
'''%(matchcount)d %(matches)s for "%(title)s"''':
'''%(matchcount)d %(matches)s para "%(title)s"''',
'''Local Site Map for "%s"''':
'''Mapa local do site para "%s"''',
'''Please log in first.''':
'''Por favor faça o login antes de tudo.''',
'''Please first create a homepage before creating additional pages.''':
'''Por favor primeiro crie uma página antes de criar uma página adicional.''',
'''MyPages management''':
'''Gerenciando MinhasPáginas''',
'''Rename Page''':
'''Renomear Página''',
'''New name''':
'''Novo nome''',
'''Optional reason for the renaming''':
'''Razão para renomear página (opcional)''',
'''(including %(localwords)d %(pagelink)s)''':
'''(incluindo %(localwords)d %(pagelink)s)''',
'''The following %(badwords)d words could not be found in the dictionary of %(totalwords)d words%(localwords)s and are highlighted below:''':
'''As seguintes %(badwords)d palavras não foram encontradas no dicionário com %(totalwords)d palavras%(localwords)s e estão destacadas abaixo:''',
'''Add checked words to dictionary''':
'''Adicionar palavras selecionadas ao dicionário''',
'''No spelling errors found!''':
'''Não foram encontrados erros de ortografia!''',
'''You can\'t check spelling on a page you can\'t read.''':
'''Você não pode verificar a ortografia de uma página que você não tem permissão para visualizar.''',
'''Do it.''':
'''Faça-o''',
'''Execute action %(actionname)s?''':
'''Executou a ação %(actionname)s?''',
'''Action %(actionname)s is excluded in this wiki!''':
'''Ação %(actionname)s foi excluída desta wiki!''',
'''You are not allowed to use action %(actionname)s on this page!''':
'''Você não tem permissão para fazer a ação %(actionname)s nesta página!''',
'''Please use the interactive user interface to use action %(actionname)s!''':
'''Por favor use a interface interativa para fazer a ação %(actionname)s!''',
'''Title Search: "%s"''':
'''Buscar Título: "%s"''',
'''Full Text Search: "%s"''':
'''Busca Completa de Texto: "%s"''',
'''Full Link List for "%s"''':
'''Lista completa de links para "%s"''',
'''Cannot create a new page without a page name.  Please specify a page name.''':
'''Não é possível criar uma página sem um nome. Por favor, especifique um nome para a página.''',
'''Pages''':
'''Páginas''',
'''Select Author''':
'''Selecione Autor''',
'''Revert all!''':
'''Reverter tudo!''',
'''You are not allowed to use this action.''':
'''Você não tem permissão para fazer esta ação.''',
'''Subscribe users to the page %s''':
'''Inscrever usuários na página %s''',
'''Subscribed for %s:''':
'''Inscrito para %s:''',
'''Not a user:''':
'''Não a usuário:''',
'''You are not allowed to perform this action.''':
'''Você não tem permissão para executar esta ação.''',
'''You are now logged out.''':
'''Você não está mais logado.''',
'''Delete''':
'''Apagar''',
'''Optional reason for the deletion''':
'''Razão para a remoção (opcional)''',
'''Really delete this page?''':
'''Realmente remover esta página?''',
'''Restored Backup: %(filename)s to target dir: %(targetdir)s.
Files: %(filecount)d, Directories: %(dircount)d''':
'''Restaurar Backup: %(filename)s foi para o diretório: %(targetdir)s.
Arquivos: %(filecount)d, Diretórios: %(dircount)d''',
'''Restoring backup: %(filename)s to target dir: %(targetdir)s failed.''':
'''Restaurando backup: %(filename)s foi para o diretório: %(targetdir)s falhou.''',
'''Wiki Backup / Restore''':
'''Wiki Backup / Restaurar''',
'''Backup''':
'''Barkup''',
'''Restore''':
'''Restaurar''',
'''You are not allowed to do remote backup.''':
'''Você não tem permissão para fazer um backup remoto.''',
'''Unknown backup subaction: %s.''':
'''Subação de backup desconhecida: %s.''',
'''[%d attachments]''':
'''[%d arquivos anexados]''',
'''There are <a href="%(link)s">%(count)s attachment(s)</a> stored for this page.''':
'''Existe(m) <a href="%(link)s">%(count)s anexo(s)</a> armazenados para esta página.''',
'''Filename of attachment not specified!''':
'''Nome do arquivo anexado não especificado!''',
'''Attachment \'%(filename)s\' does not exist!''':
'''Arquivo anexado %(filename)s não existe!''',
'''To refer to attachments on a page, use \'\'\'{{{attachment:filename}}}\'\'\', 
as shown below in the list of files. 
Do \'\'\'NOT\'\'\' use the URL of the {{{[get]}}} link, 
since this is subject to change and can break easily.''':
'''Para se referi aos anexos de uma página, use \'\'\'{{{attachment:filename}}}\'\'\', 
como mostrado abaixo na lista de arquivos. 
\'\'\'NÃO\'\'\' use a URL do link {{{[get]}}}, 
já que a mesma está sujeita a alterações, e pode facilmente ser inutilizada.''',
'''unzip''':
'''não-zipado''',
'''install''':
'''instalar''',
'''No attachments stored for %(pagename)s''':
'''Nenhum anexo armazenado para %(pagename)s''',
'''Edit drawing''':
'''Editar desenho''',
'''Attached Files''':
'''Arquivos Anexados''',
'''You are not allowed to attach a file to this page.''':
'''Você não tem permissão para anexar arquivos a esta página.''',
'''New Attachment''':
'''Novo Anexo''',
'''An upload will never overwrite an existing file. If there is a name
conflict, you have to rename the file that you want to upload.
Otherwise, if "Rename to" is left blank, the original filename will be used.''':
'''Um upload nunca irá sobrescrever um arquivo. Se existir um conflito quanto
ao nome, você será lembrado sobre o nome do arquivo que você deseja inserir.
Por outro lado, se "Renomear como" for deixado em branco, o nome original do arquivo será usado.''',
'''File to upload''':
'''Arquivo para enviar''',
'''Rename to''':
'''Renomear para''',
'''Upload''':
'''Enviar''',
'''File attachments are not allowed in this wiki!''':
'''Arquivos anexados não são permitidos nesta wiki!''',
'''You are not allowed to save a drawing on this page.''':
'''Você não tem permissão para salvar um desenho nesta página.''',
'''No file content. Delete non ASCII characters from the file name and try again.''':
'''Sem arquivo. Apague os caracteres não ASCII do nome do arquivo e tente novamente.''',
'''You are not allowed to delete attachments on this page.''':
'''Você não tem permissão para remover arquivos anexados a esta página.''',
'''You are not allowed to get attachments from this page.''':
'''Você não tem permissão para acessar arquivos anexados a esta página.''',
'''You are not allowed to unzip attachments of this page.''':
'''Você não tem permissão para anexar arquivos não-zipados a esta página.''',
'''You are not allowed to install files.''':
'''Você não tem permissão para adicionar arquivos!''',
'''You are not allowed to view attachments of this page.''':
'''Você não tem permissão para ver arquivos anexados a esta página.''',
'''Unsupported upload action: %s''':
'''Ação não permitida para envio de arquivo: %s''',
'''Attachments for "%(pagename)s"''':
'''Arquivos anexados para "%(pagename)s"''',
'''Attachment \'%(target)s\' (remote name \'%(filename)s\') with %(bytes)d bytes saved.''':
'''Anexo \'%(target)s\' (remote name \'%(filename)s\') com %(bytes)d bytes salvo.''',
'''Attachment \'%(filename)s\' deleted.''':
'''Anexo \'%(filename)s\' apagado.''',
'''Attachment \'%(filename)s\' installed.''':
'''Anexo \'%(filename)s\' adicionado.''',
'''Attachment \'%(filename)s\' could not be unzipped because the resulting files would be too large (%(space)d kB missing).''':
'''Anexo \'%(filename)s\' não pode ser descompactado porque os arquivos resultantesseriam muito grandes (%(space)d kB missing).''',
'''Attachment \'%(filename)s\' could not be unzipped because the resulting files would be too many (%(count)d missing).''':
'''Anexo \'%(filename)s\' não pode ser descompactado porque os arquivos resultantes seriam muito grandes (%(count)d kB missing).''',
'''Attachment \'%(filename)s\' unzipped.''':
'''Anexo \'%(filename)s\' não-zipado.''',
'''Attachment \'%(filename)s\' not unzipped because the files are too big, .zip files only, exist already or reside in folders.''':
'''Anexo \'%(filename)s\' não descompactado porque o arquivo é muito grande, somente arquivos .zip que já existam ou que estejam em pastas.''',
'''The file %(target)s is not a .zip file.''':
'''O arquivo %(target)s não é um arquivo .zip.''',
'''Attachment \'%(filename)s\'''':
'''Arquivo anexado \'%(filename)s\'''',
'''Package script:''':
'''Script do pacote:''',
'''File Name''':
'''Nome do arquivo''',
'''Modified''':
'''Modificado''',
'''Unknown file type, cannot display this attachment inline.''':
'''Tipo de arquivo desconhecido. Não é possível mostrar o anexo diretamente na página.''',
'''attachment:%(filename)s of %(pagename)s''':
'''[[Verbatim(attachment:)]]%(filename)s de %(pagename)s''',
'''Upload new attachment "%(filename)s"''':
'''Enviar novo arquivo anexado "%(filename)s"''',
'''Create new drawing "%(filename)s"''':
'''Criar novo desenho "%(filename)s"''',
'''Edit drawing %(filename)s''':
'''Editar desenho %(filename)s''',
'''FrontPage''':
'''PáginaPrincipal''',
'''RecentChanges''':
'''AlteraçõesRecentes''',
'''TitleIndex''':
'''ÍndiceDeTítulos''',
'''WordIndex''':
'''ÍndiceDePalavras''',
'''FindPage''':
'''EncontrarPágina''',
'''SiteNavigation''':
'''NavegaçãoNoSite''',
'''HelpContents''':
'''ConteúdoDaAjuda''',
'''HelpOnFormatting''':
'''AjudaNaFormatação''',
'''UserPreferences''':
'''PreferênciasDoUsuário''',
'''WikiLicense''':
'''LicençaWiki''',
'''MissingPage''':
'''PáginaFaltante''',
'''MissingHomePage''':
'''HomePageFaltante''',
'''Mon''':
'''Seg''',
'''Tue''':
'''Ter''',
'''Wed''':
'''Qua''',
'''Thu''':
'''Qui''',
'''Fri''':
'''Sex''',
'''Sat''':
'''Sáb''',
'''Sun''':
'''Dom''',
'''AttachFile''':
'''AnexarArquivo''',
'''DeletePage''':
'''ApagarPágina''',
'''LikePages''':
'''PáginasSemelhantes''',
'''LocalSiteMap''':
'''MapaLocalDoSite''',
'''RenamePage''':
'''RenomearPágina''',
'''SpellCheck''':
'''VerificaçãoOrtográfica''',
'''Invalid MonthCalendar calparms "%s"!''':
'''Parâmetros MêsCalendário inválidos "%s"!''',
'''Invalid MonthCalendar arguments "%s"!''':
'''Argumentos MêsCalendário inválidos "%s"!''',
'''Unsupported navigation scheme \'%(scheme)s\'!''':
'''Esquema de navegação %(scheme)s não permitido!''',
'''No parent page found!''':
'''Não foi encontrada página pai!''',
'''Wiki''':
'''Wiki''',
'''Slideshow''':
'''Mostra em slide''',
'''Start''':
'''Iniciar''',
'''Slide %(pos)d of %(size)d''':
'''Slide %(pos)d de %(size)d''',
'''No orphaned pages in this wiki.''':
'''Nenhuma página órfã neste wiki.''',
'''No quotes on %(pagename)s.''':
'''Sem aspas em %(pagename)s''',
'''Upload of attachment \'%(filename)s\'.''':
'''Envio de anexo \'%(filename)s\'.''',
'''Drawing \'%(filename)s\' saved.''':
'''Desenho \'%(filename)s\' salvo.''',
'''%(mins)dm ago''':
'''%(mins)dm atrás''',
'''(no bookmark set)''':
'''(nenhum marcador configurado)''',
'''(currently set to %s)''':
'''(atualmente configurado para %s)''',
'''Delete Bookmark''':
'''Apagar Bookmark''',
'''Set bookmark''':
'''Ajustar Bookmark''',
'''set bookmark''':
'''ajustar bookmark''',
'''[Bookmark reached]''':
'''[Bookmark atingido]''',
'''Contents''':
'''Conteúdos''',
'''No wanted pages in this wiki.''':
'''Nenhuma página solicitada neste wiki.''',
'''Invalid include arguments "%s"!''':
'''Parâmetros de inclusão inválidos "%s"!''',
'''Nothing found for "%s"!''':
'''Nada encontrado para "%s"!''',
'''Markup''':
'''Markup''',
'''Display''':
'''Mostrar''',
'''Filename''':
'''Nome do arquivo''',
'''Rendering of reStructured text is not possible, please install docutils.''':
'''Renderização de texto reStructured não é possível, por favor instale o docutils.''',
'''**Maximum number of allowed includes exceeded**''':
'''**O número máximo de inserções permitidas foi excedida**''',
'''**Could not find the referenced page: %s**''':
'''**Não foi possível encontrar a página referenciada: %s**''',
'''Expected "%(wanted)s" after "%(key)s", got "%(token)s"''':
'''Esperado "%(wanted)s" depois de "%(key)s", obtido "%(token)s"''',
'''Expected an integer "%(key)s" before "%(token)s"''':
'''Esperado um inteiro "%(key)s" antes de "%(token)s"''',
'''Expected an integer "%(arg)s" after "%(key)s"''':
'''Esperado um inteiro "%(arg)s" depois de "%(key)s"''',
'''Expected a color value "%(arg)s" after "%(key)s"''':
'''Esperado um valor de cor "%(arg)s" depois de "%(key)s"''',
'''XSLT option disabled, please look at HelpOnConfiguration.''':
'''A opção XSLT está desabilitada, por favor vejja AjudaNaConfiguração.''',
'''XSLT processing is not available, please install 4suite 1.x.''':
'''O processamento de XSLT não está disponível, por favor instale 4suite 1.x.''',
'''%(errortype)s processing error''':
'''%(errortype)s erros de processamento''',
'''Views/day''':
'''Visualizações/dia''',
'''Edits/day''':
'''Edições/dia''',
'''%(chart_title)s for %(filterpage)s''':
'''%(chart_title)s para %(filterpage)s''',
'''green=view
red=edit''':
'''verde=ver
vermelho=editar''',
'''date''':
'''data''',
'''# of hits''':
'''# de hits''',
'''Page Size Distribution''':
'''Distribuição do Tamanho de Página''',
'''page size upper bound [bytes]''':
'''limite superior para o tamanho da página [bytes]''',
'''# of pages of this size''':
'''# páginas deste tamanho''',
'''User agent''':
'''Agente usuário''',
'''Others''':
'''Outros''',
'''Distribution of User-Agent Types''':
'''Distribuição dos Tipos de Agentes de Usuários''',
'''Unsubscribe''':
'''Remover subscrição''',
'''Home''':
'''Home''',
'''[RSS]''':
'''[RSS]''',
'''[DELETED]''':
'''[APAGADO]''',
'''[UPDATED]''':
'''[ATUALIZADO]''',
'''[NEW]''':
'''[NOVO]''',
'''[DIFF]''':
'''[DIFF]''',
'''[BOTTOM]''':
'''[FINAL]''',
'''[TOP]''':
'''[TOPO]''',
'''Click to do a full-text search for this title''':
'''Clique aqui para fazer uma busca completa para este título''',
'''Preferences''':
'''Preferências''',
'''Logout''':
'''Sair''',
'''Clear message''':
'''Apagar mensagem''',
'''last edited %(time)s by %(editor)s''':
'''editada pela última vez em %(time)s por %(editor)s''',
'''last modified %(time)s''':
'''modificada pela última vez em %(time)s''',
'''Search:''':
'''Procurar:''',
'''Text''':
'''Texto''',
'''Titles''':
'''Títulos''',
'''Search''':
'''Procurar''',
'''More Actions:''':
'''Mais Ações:''',
'''------------''':
'''------------''',
'''Raw Text''':
'''Visualizar Texto''',
'''Delete Cache''':
'''Deletar Cache''',
'''Delete Page''':
'''Apagar Página''',
'''Like Pages''':
'''Páginas Semelhantes''',
'''Local Site Map''':
'''Mapa do Site Local''',
'''My Pages''':
'''Minhas Páginas''',
'''Subscribe User''':
'''Inscrever usuário''',
'''Remove Spam''':
'''Remover Spam''',
'''Package Pages''':
'''Páginas do Pacote''',
'''Render as Docbook''':
'''Renderizar como Docbook''',
'''Do''':
'''Fazer''',
'''Edit (Text)''':
'''Editar (Texto)''',
'''Edit (GUI)''':
'''Editar (GUI)''',
'''Immutable Page''':
'''Página não Editável''',
'''Remove Link''':
'''Remover Link''',
'''Add Link''':
'''Adicionar Link''',
'''Attachments''':
'''Anexos''',
'''Show %s days.''':
'''Mostrar %s dias.''',
'''DeleteCache''':
'''ApagarCache''',
'''(cached %s)''':
'''(armazenado %s)''',
'''Or try one of these actions:''':
'''Ou tente uma da seguintes ações: ''',
'''Page''':
'''Página''',
'''User''':
'''Usuário''',
'''Sorry, can not save page because "%(content)s" is not allowed in this wiki.''':
'''Desculpe, mas não é possível salvar esta página porque"%(content)s" não é permitido nesta wiki.''',
'''Line''':
'''Linha''',
'''Deletions are marked like this.''':
'''Deleções são marcadas assim.''',
'''Additions are marked like this.''':
'''Adições são marcadas assim.''',
'''Connection to mailserver \'%(server)s\' failed: %(reason)s''':
'''Conexão ao servidor de correio \'%(server)s\' falhou: %(reason)s''',
'''Mail not sent''':
'''E-mail não enviado''',
'''Mail sent OK''':
'''E-mail enviado''',
}
