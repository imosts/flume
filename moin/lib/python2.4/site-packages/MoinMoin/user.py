...
def getUserList(request):
    ...
    @return: all user IDs
    """
    import re, dircache, error
    raise error.UnsupportedError, "This is not supported by flumeed_moin"
...
def getUserId(request, searchName):
    ...
    """
    import flmw, flmw.util
    try:
        id = flmw.util.username2uid (searchName, request.cfg.usermgruid)
        return id
    except OSError, err:
        import errno
...
def getUserIdentification(request, username=None):
    ...
    """
    import error
    raise LoginError, "This is not supported by flumeed_moin"
    _ = request.getText
...
class User:
    ...
    def __init__(self, request, id=None, name="", password=None, auth_username="", **kw):
        ...
        self._flumeuser = None
        self.valid = 0
        self.trusted = 0
        self.setuid (id, request)
        ...
        #self.edit_cols = 80
        self.flm_offset = int(float(self._cfg.flm_offset) * 3600)
        self.language = ""
        self.date_fmt = ""
        self.datetime_fmt = ""
        self.quicklinks = []
    ...
    def homedir(self):
        slab = flmo.Label ([self._flumeuser._e_tag])
        ls = flmo.LabelSet (S=slab, I=self._cfg.moin_i_lab)
        return os.path.join (self._cfg.user_dir, ls.to_filename())
    ...
    def __filename(self):
        ...
        """
        return os.path.join(self.homedir(), "profile")

    def __bookmark_filename(self):
        if self._cfg.interwikiname:
            import error
            raise error.UnsupportedError, "flumeed moin does not support interwiki?"
        else:
            return os.path.join (self.homedir(), "bookmark")
    ...
    def load_from_id(self, check_pass=0):
        ...
        if not self.exists():
            import error
            raise error.LoginError, "user does not exist?"
            return
        ...
        self.flm_offset = int(self.flm_offset)
        ...
        if -24 <= self.flm_offset and self.flm_offset <= 24:
            self.flm_offset = self.flm_offset * 3600
    ...
    def flumeuser(self, uid, request):
        if not self._flumeuser:
            import flmw, flmw.user
            p = flmw.user.User(uid = uid.decode('utf-8').encode('ascii', 'replace'))
            p.readTags ()
            self._flumeuser = p
        return self._flumeuser

    def setuid(self, uid, request=None):
        self.id = uid
        if (uid):
            try:
                self.flumeuser (uid, request)
            except flume.DeclassificationError, err:
                self.id = None # could not read tags file.

    def augment_S (self):
        u = self.flumeuser (self.id, self._request)
            
        self._orig_S = flmo.get_label (flume.LABEL_S)
        self._changed_Slabel = False

        if u._e_tag not in self._orig_S:
            self._changed_Slabel = True
            newlabel = flmo.Label([u._e_tag])
            newlabel += self._orig_S.toList ()
            flmo.set_label(flume.LABEL_S, newlabel)

    def restore_S (self):
        if self._changed_Slabel:
            flmo.set_label(flume.LABEL_S, self._orig_S)

    def augment_I (self):
        self._orig_I = flmo.get_label (flume.LABEL_I)
        self._changed_Ilabel = False
        
        if self._cfg.moin_i_lab[0] not in self._orig_I:
            self._changed_Ilabel = True
            newlabel = flmo.Label([self._cfg.moin_i_lab[0]])
            newlabel += self._orig_I.toList ()
            flmo.set_label(flume.LABEL_I, newlabel)
        
    def restore_I (self):
        if self._changed_Ilabel:
            flmo.set_label(flume.LABEL_I, self._orig_I)

    def save(self):
        """ Save user account data to user account file on disk.

        This saves all member variables, except "id" and "valid" and
        those starting with an underscore.  If slabel is supplied, it will
        create the files with S=<slabel>.
        """
        if not self.id:
            return
        ...
        #We could make the 'users' directory, but its probably there already
        #filesys.makeDirs(user_dir) 

        self.augment_S ()
        self.augment_I ()
        f = open (self.__filename(), "w")
        data = codecs.EncodedFile (f, config.charset)
    ...
    def getTime(self, tm):
        ...
        """
        return timefuncs.tmtuple(tm + self.flm_offset)
    ...
    def addTrail(self, pagename):
        ...
        """
        if not self._request.cfg.use_trails:
            return
    ...
    def saveTrail(self):
        ...
        data = '\n'.join(self._trail) + '\n'
        path = os.path.join (self.homedir(), "trail")
        try:
    ...
    def getTrail(self):
        ...
        trailpath = os.path.join (self.homedir(), "trail")
        if self.valid and (self.show_page_trail or self.remember_last_visit) \
                and not self._trail \
                and os.path.exists(trailpath):
            try:
                trail = codecs.open(trailpath, 'r', config.charset).readlines()
            except (OSError, ValueError):
                trail = []
