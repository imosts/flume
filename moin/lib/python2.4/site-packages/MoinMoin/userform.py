...
Contact the owner of the wiki, who can enable email.""")
        ...
        if (form.has_key('create') or
            ...
            return ("You can't do this anymore, please use moinlaunch to create users ?action=createuser")
...
space between words. Group page name is not allowed.""") % wikiutil.escape(theuser.name)
            ...
            # time zone
            theuser.flm_offset = util.web.getIntegerInput(self.request, 'flm_offset', theuser.flm_offset, -84600, 84600)
            ...
            already_handled = ['name', 'password', 'password2', 'email',
                               ...
                               'editor_ui', 'flm_offset', 'datetime_fmt',
                               'theme_name', 'language']
...
class UserSettings:
    ...
    def _flm_select(self):
        """ Create time zone selection. """
        flm = 0
        if self.request.user.valid:
            flm = int(self.request.user.flm_offset)
        ...
        return util.web.makeSelection('flm_offset', options, str(flm))
    ...
    def _theme_select(self):
        ...
        for theme in self.request.cfg.themes.keys():
    ...
    def asHTML(self, create_only=False):
        ...
        if self.request.user.valid and not create_only:
            ...
            if not "flm_offset" in self.cfg.user_form_remove:
...
class Login:
    ...
    def asHTML(self):
        ...
        self._form.append(html.INPUT(type="hidden", name="umgr", value=request.cfg.usermgr))
