...
if __name__ == "__main__":
    ...
    # Hack hack hack
    def print_py_files (include_prefix=True, include_pyc=False):
        for dirpath, dirnames, filenames in os.walk ("PIL"):
            for f in filenames:
                if f.endswith ('.py'):
                    if include_prefix:
    ...
    def print_csrc_files (include_prefix=True):
    ...
    if len (sys.argv) > 1 and sys.argv[1] == 'list_inst_files':
        print_py_files (include_prefix=False, include_pyc=True)
        print_so_files ()
        sys.exit (0)
