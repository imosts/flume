...
function deserialize_data (d) {
  data = d.split (ROW_DELIM);
}
function format_post (user, email, msg, img_lnk) {
  var msg_id = data.length + 1;
  var chat_id = 1225568854;
  
  return (msg_id + COL_DELIM +
          chat_id + COL_DELIM +
          (new Date ()).getTime () + COL_DELIM +
          user + COL_DELIM +
          "1" + COL_DELIM +
          "mailto:" + email + COL_DELIM +
          msg + COL_DELIM +
          img_lnk);
}
...
function do_post(){
  ...
  f.pst.value="";
  f.pst.focus();
  data.push (format_post (f.nme.value, "foo@mit.edu", ptmp, ""));
...
function ar_check(s){
  if(!http("GET",
           "./?"+s_rq+"&sec=ar&p="+lp+"&c="+((new Date()).getTime()),
           null,
           "aj_proc",
           s)){
...
function add_post(t){
  ...
  if(t[0]==0||t[0]==-1){
    ...
    if (s_av&&t[7]) x += '<img src="http://'+t[7]+'" class="pic">';
    else if (s_av&&(s=='.gif' || s=='.jpg' || s=='.png')) {x += '<img src="'+t[5]+'" class="pic">';t[5] = '';}
    if(s_dt>1){
...
function upd_tms(c){
  ...
  if(c===undefined){
    ts = 0;
    if (td = getcookie('td')) ts = td;
    c = parseInt((new Date()).getTime()/1000)-ts;
  }
  tarr=t16;
...
function delban(){
  ...
  for(i=0;i<cn;i++){
    ...
    if((mod||(s_ld&&a.id==mylp&&mylp==lp&&lvl>1))){
      if(!lnkd[a.id]){
        lnkd[a.id]=true;
        b.innerHTML=b.innerHTML.replace(/(div>|pic\">|^)<(a|b)/i,"$1<span><a href=\"JavaScript:parent.cboxform.del("+a.id+")\" title=\""+t17+"\">[&times;]</a>&nbsp;"+((mod)?"<a href=\"JavaScript:parent.cboxform.ban("+a.id+")\" title=\""+t18+"\" onMouseOver=\"JavaScript:parent.cboxform.getip("+a.id+", this)\">[o]</a>&nbsp;":"")+"</span><$2");
      }
    }
  }
}
