/* $Id: slave.C 1682 2006-04-26 19:17:22Z max $ */
...
pub_accept (pubserv_cb cb, int pubfd)
{
  ...
  if (fd >= 0) {
    ...
    tcp_nodelay (fd);
    ref<axprt_stream> x = axprt_stream::alloc (fd, flume_ps);
    (*cb) (x);
...
pub_accept_unix (pubserv_cb cb, int pubfd)
{
  ...
  int fd = _pub_accept (pubfd, &sin);
  if (fd >= 0) {
    ref<axprt_stream> x = axprt_unix::alloc (fd, flume_ps);
    (*cb) (x);
  } else if (errno != EAGAIN)
...
helper_exec_t::launch (cbb c)
{
  ...
  argv[0] = prog;
  ptr<axprt_unix> ux = 
    axprt_unix_aspawnv (prog, argv, flume_ps, 
...
helper_t::mksrv ()
{
  if (!_srvcb)
    return true;
  _srv = asrv::alloc (x, rpcprog, 
		      wrap (this, &helper_t::dispatch, destroyed));
  assert (_srv);
  return true;
}

void
helper_t::dispatch (ptr<bool> df, svccb *sbp)
{
  if (!sbp) {
    eofcb (df);
    return;
  ...
  if (!_srvcb) {
    warn ("Helper_t::dispatch without a _srvcb!!\n");
  } else {
    (*_srvcb) (sbp);
  }
}
...
helper_unix_t::launch (cbb cb)
{
  ...
  x = _unix_x = axprt_unix::alloc (fd, flume_ps);
  mkclnt ();
  mksrv ();
  ping (cb);
}
...
helper_inet_t::launch_cb (cbb c, ptr<bool> df, int f)
{
  ...
  if (f < 0) {
    ...
    fd = f;
    close_on_exec (fd);
    if (!(x = axprt_stream::alloc (fd, flume_ps)) || !mkclnt () || !mksrv ())
      ret = false;
  }
  if (!ret)
    (*c) (ret);
		...
		time_t duration)
{
  ...
  if (status != HLP_STATUS_OK || calls >= max_calls) {
	...
	_qlen < max_qlen) {
      queued_call_t *c = New queued_call_t (procno, in, out, cb, duration);
      _queue.insert_tail (c);
      canc = c->cancel_button ();
      _qlen ++;
    } else {
      (*cb) (RPC_CANTSEND);
    }
...
{
  while (calls < max_calls && _queue.first && status == HLP_STATUS_OK) {
    queued_call_t *qc = _queue.first;
    _queue.remove (qc);
    qc->call (this);
    delete qc;
  }
...
void 
helper_t::set_srvcb (asrv_cb_t::ptr c)
{
  bool makeit = false;
  if (!_srvcb && c) {
    makeit = true;
