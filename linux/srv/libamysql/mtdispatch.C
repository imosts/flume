/* $Id: mtdispatch.C 1618 2006-03-09 23:19:56Z max $ */
...
ssrv_client_t::ssrv_client_t (ssrv_t *s, const rpc_program *const p, 
  ...
  : ssrv (s), txa_prog (t), _destroyed (New refcounted<bool> (false))
{
  ssrv->insert (this);
...
ssrv_client_t::~ssrv_client_t () 
{ 
	ssrv->remove (this); 
	ssrv = (ssrv_t *)0xcfcfcfcf;
	*_destroyed = true;
}
  ...
  : mtd (NULL), prog (&p), load_avg (0), txa_prog (x)
{
  ...
  if (!mtd) {
#ifdef HAVE_PTH
    warn << "Requested thread type not available: using GNU Pth\n";
    mtd = New mgt_dispatch_t (c, n, m, this, x);
#else
# ifdef HAVE_PTHREADS
    warn << "Requested thread type not available: using POSIX pthreads\n";
    mtd = New mpt_dispatch_t (c, n, m, this, x);
# else
#  ifdef HAVE_KTHREADS 
    warn << "Requested thread type not available: using kernel threads\n";
    mtd = New mkt_dispatch_t (c, n, m, this, x);
#  else
...
ssrv_t::req_made ()
{
  ...
  while (reqtimes.size () && tsdiff (reqtimes[0], sfs_get_tsnow(), 
				     ok_amt_lasi))
    reqtimes.pop_front ();
