// -*-c++-*-
/* $Id: amysql.h 1715 2006-04-30 17:16:14Z max $ */
...
public:
  ...
  mybind_rpcstr_t (const rpc_opaque<n> &in)
    : mybind_t (MYSQL_TYPE_STRING),
      mys (str (in.base(), in.size ()), MYSQL_TYPE_STRING, true) {}
#ifdef HAVE_MYSQL_BIND
...
public:
  ...
  template<size_t n>
  mybind_param_t (const rpc_opaque<n> &b)
  { p = New refcounted<mybind_rpcstr_t<n> > (b); }
  ...
  template<size_t n>
  mybind_param_t &operator = (const rpc_opaque<n> &b)
  {
    str s (b.base (), b.len ());
    p = New refcounted<mybind_str_t> (s); return (*this);
  }
